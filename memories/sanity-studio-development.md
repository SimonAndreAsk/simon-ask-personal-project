...
**Desk:** Drafts / Published / All posts filter `_type == "post"`. New types → `S.listItem()` + `S.documentList()` in `structure.ts`.

**Presentation:** `post` → `/${slug.current}` and `/` in `presentation/resolve.ts`. New previewable types need a `locations.<type>` entry and a real Next route. Preview URLs: `sanity.config.ts`.
